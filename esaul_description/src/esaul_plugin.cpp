...
namespace gazebo
{
	class ModelPush : public ModelPlugin
	{
		...
		void Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
			{
				...
				IMU_odom_sub_ = nh.subscribe("/IMU/odom", 50, &ModelPush::IMU, this);
		...
		void updateOdom()
			{
				...
				if(delta_time>0)
				{
					...
					lin_speed_enc_x = (right_speed + left_speed) * 0.5;	
					lin_speed_enc_y = (right_speed + left_speed) * sin(alpha);

					ang_speed_enc_z = (right_speed - left_speed)/width;

					IMU_x=IMU_x+imu_lin_acel_x*delta_time*delta_time; 
					IMU_y=IMU_y+imu_lin_acel_y*delta_time*delta_time;
					IMU_a=IMU_a+imu_ang_vel_z*delta_time;	
		 ...
		 void FileWrite()
			{
				...
				ofstream out ("/home/yan/catkin_ws/src/esaul_description/file.txt", ios::app);
				if (out.is_open())
				{
					out << real_x << " " << real_y << " " << x_lazer << " " << y_lazer <<  " " << lin_speed_enc_x << " " << x_nonoise << " " << y_nonoise << " " << delta_time << " " << speed_r << " " << speed_l << std::endl;
				}
				out.close();
			...
			void IMU(const geometry_msgs::Vector3 &msg)  
			{  
				//imu_lin_acel_x = msg.linear_acceleration.x;
				//imu_lin_acel_y = msg.linear_acceleration.y;
		...
		double IMU_x=0, IMU_y=0, IMU_a=0;
		double imu_ang_vel_z=0, imu_lin_acel_y=0, imu_lin_acel_x=0;
